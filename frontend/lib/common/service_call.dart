...
import 'package:taxi_app/model/service_detail_model.dart';
import 'package:taxi_app/model/zone_document_model.dart';
...
class ServiceCall {
  ...
  static Future<Map<String, dynamic>> postWithResponse(
    Map<String, dynamic> parameter,
    String path, {
    bool isTokenApi = false,
  }) async {
  ...
  static void getStaticDateApi() {
    post({"last_call_time": ""}, SVKey.svStaticData,
      ...
      try {
        if (responseObj[KKey.status] == "1") {
          ...
          for (var dObj in (payload["zone_document"] as List? ?? [])) {
            batch?.insert(
                DBHelper.tbZoneDocument, ZoneDocumentModel.map(dObj).toMap(),
                conflictAlgorithm: ConflictAlgorithm.replace);
          }
