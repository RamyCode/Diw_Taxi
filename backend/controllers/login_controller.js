// File: taxi_driver_node-main/controllers/login_controller.js
...
module.exports.controller = (app, io, socket_list) => {
    ...
    // ✅ اجلب عنوان IP من الطلب
    const userIp = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    console.log("User login IP:", userIp);

    helper.CheckParameterValid(res, reqObj, ["user_type", "mobile_code", "mobile", "os_type", "push_token", "socket_id"], () => {

        db.query(
            'SELECT `user_id`, `name`, `email`, `gender`, `mobile`, `mobile_code`, `user_type`, `is_block`, `image`, `status` FROM `user_detail` WHERE `mobile` = ? AND `mobile_code` = ? AND `user_type` = ?',
            [reqObj.mobile, reqObj.mobile_code, reqObj.user_type],
            (err, result) => {
                if (err) {
                ...
                if (result.length > 0) {
                    ...
                    console.log("Updating push_token for", reqObj.mobile, reqObj.mobile_code, reqObj.user_type);
                    console.log("push_token received:", reqObj.push_token);

                    db.query(
                        "UPDATE `user_detail` SET `auth_token` = ?, `push_token` = ?, `modify_date` = NOW(), `last_login_ip` = ? WHERE `mobile` = ? AND `mobile_code` = ? AND `user_type` = ?",
                        [auth_token, reqObj.push_token, userIp, reqObj.mobile, reqObj.mobile_code, reqObj.user_type],
                        (err, uResult) => {
                            if (err) {
                                helper.ThrowHtmlError(err, res);
                                return;
                            ...
                            console.log("UPDATE executed. Affected Rows:", uResult.affectedRows);
                    ...
                    db.query(
                        "INSERT INTO `user_detail` (`mobile`, `mobile_code`, `user_type`, `push_token`, `auth_token`, `device_source`, `status`, `last_login_ip`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        [reqObj.mobile, reqObj.mobile_code, reqObj.user_type, reqObj.push_token, auth_token, reqObj.os_type, "1", userIp],
                        (err, result) => {
                            if (err) {
                                helper.ThrowHtmlError(err, res);
                                return;
...
///عدم تسجيل دخول
                    res.json({
                  "status": "0",
                     "message": "هذا الرقم غير مسجل، الرجاء إنشاء حساب أولاً"
                     });
                }
            })
        })
