// File: taxi_driver_node-main/controllers/booking_controller.js
// في الأعلى: تأكد من استيراد axios

const { insertBookingEventLog } = require('./../helpers/helpers') // إن لم تكن مضافة
...
module.exports.controller = (app, io, socket_list) => {
                            ...
                            if (result[1].length > 0) {
                                ...
                                db.query("INSERT INTO `payment_detail` ( `card_id`, `payment_type`, `amt`, `discount_amt`, `driver_amt`, `tax_amt`, `ride_commission`,  `created_date`, `modify_date` ) VALUES (?,?,?, ?,?,?, ?,NOW(),NOW()) ", [
                                    ...
                                    if (pResult) {
                                        ...
                                        db.query(
                                            "INSERT INTO `booking_detail`( `user_id`, `pickup_lat`, `pickup_long`, `pickup_address`, `drop_lat`, `drop_long`, `drop_address`, `pickup_date`, `service_id`, `price_id`, `payment_id`, `est_total_distance`, `est_duration`, `user_comment`,  `created_date`) VALUES (?,?,?, ?,?,?, ?,?,?,  ?,?,?, ?, ?, NOW())", [

                                            uObj.user_id, reqObj.pickup_latitude, reqObj.pickup_longitude,

                                            reqObj.pickup_address, reqObj.drop_latitude, reqObj.drop_longitude, reqObj.drop_address, reqObj.pickup_date, reqObj.service_id, reqObj.price_id, pResult.insertId, reqObj.est_total_distance, reqObj.est_duration, reqObj.user_comment || "",
    ...
    (err, result) => {
        ...
        if (result.length > 0) {
            const bookingData = result[0]; // خزن نسخة من البيانات

            driverNewRequestSend(bookingData, (status, message) => {
                helper.Dlog("-------- new request send callback -------");

                res.json({
                    status: status == 1 ? "1" : "2",
                    payload: bookingData,
                    message: status == 1 ? "booking request send successfully" : message
                });
            });
...
app.post('/api/estimate_fare', (req, res) => {
    const reqObj = req.body;

    const priceId = parseInt(reqObj.price_id);
    const zoneId = parseInt(reqObj.zone_id);
    const serviceId = parseInt(reqObj.service_id);
    const estDistance = parseFloat(reqObj.est_total_distance);
    const estDuration = parseFloat(reqObj.est_duration);

    // التحقق من صحة القيم
    if (
        !priceId || !zoneId || !serviceId ||
        isNaN(estDistance) || isNaN(estDuration)
    ) {
        return res.json({ status: "0", message: "Invalid input for fare estimation" });
    }

    // التحقق من أن price_id فعلاً يخص zone_id و service_id
    db.query(
        "SELECT base_charge, per_km_charge, per_min_charge, booking_charge, mini_fair, mini_km, tax " +
        "FROM price_detail WHERE price_id = ? AND zone_id = ? AND service_id = ? AND status = 1",
        [priceId, zoneId, serviceId],
        (err, result) => {
            if (err) {
                helper.ThrowHtmlError(err, res);
            ...
            if (result.length === 0) {
                return res.json({ status: "0", message: "Price detail not found or not valid for zone/service" });
            }

            const pd = result[0];
            const base = parseFloat(pd.base_charge);
            const perKm = parseFloat(pd.per_km_charge);
            const perMin = parseFloat(pd.per_min_charge);
            const booking = parseFloat(pd.booking_charge);
            const miniFair = parseFloat(pd.mini_fair);
            const miniKm = parseFloat(pd.mini_km);
            const taxRate = parseFloat(pd.tax);

            let fare = base + booking;

            if (estDistance > miniKm) {
                fare += (estDistance - miniKm) * perKm;
            }

            fare += estDuration * perMin;

            if (fare < miniFair) {
                fare = miniFair;
            }

            const taxAmount = (fare * taxRate) / 100;
            const finalAmount = fare + taxAmount;

            res.json({
                status: "1",
                base_amount: fare.toFixed(2),
   ...
   app.post('/api/ride_start', (req, res) => {
    ...
    checkAccessToken(req.headers, res, (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["booking_id", "pickup_latitude", "pickup_longitude", "otp_code"], () => {
            ...
            var sqlSelect = "SELECT bd.*, pd.* FROM booking_detail AS bd " +
                "INNER JOIN payment_detail AS pd ON pd.payment_id = bd.payment_id " +
                "WHERE bd.booking_id = ? AND bd.driver_id = ? AND bd.otp_code = ? AND bd.booking_status < ?";

            db.query(sqlSelect, [reqObj.booking_id, uObj.user_id, reqObj.otp_code, bs_start], (err, result) => {
                if (err) {
                    helper.ThrowHtmlError(err, res);
                    return;
                }
                ...
                if (result.length === 0) {
                    res.json({
                        "status": "0",
                        "message": "Invalid OTP or ride already started"
                    });
                    return;
                }
                ...
                // ❌ لا حساب جديد، فقط استخدام القيم المحفوظة مسبقًا
                const totalAmount = parseFloat(booking.amt) || 0;
                const taxAmount = parseFloat(booking.tax_amt) || 0;
                const driverAmount = parseFloat(booking.driver_amt) || 0;
                const rideCommission = parseFloat(booking.ride_commission) || 0;

                const otp_code = Math.floor(1000 + Math.random() * 9000);

                const sqlUpdate = "UPDATE booking_detail AS bd " +
                    "INNER JOIN user_detail AS dd ON dd.user_id = bd.driver_id " +
                    "INNER JOIN user_detail AS ud ON ud.user_id = bd.user_id " +
                    "INNER JOIN payment_detail AS pd ON pd.payment_id = bd.payment_id " +
                    "SET bd.booking_status = ?, bd.pickup_lat = ?, bd.pickup_long = ?, bd.start_time = NOW(), " +
                    "dd.status = 2, ud.status = 2, bd.otp_code = ?, " +
                    "pd.amt = ?, pd.driver_amt = ?, pd.tax_amt = ?, pd.ride_commission = ?, pd.status = 0 " +
                    "WHERE bd.booking_id = ? AND bd.driver_id = ?";

                db.query(sqlUpdate, [
                    bs_start,
                    reqObj.pickup_latitude,
                    reqObj.pickup_longitude,
                    otp_code,
                    totalAmount,
                    driverAmount,
                    taxAmount,
    ...
    checkAccessToken(req.headers, res, (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["booking_id", "drop_latitude", "drop_longitude", "toll_tax", "ride_location"], () => {
            ...
            let rideLocationString = "";
            let rideLocationArr = JSON.parse(reqObj.ride_location);

            rideLocationArr.forEach((loc) => {
                rideLocationString += `[${loc.latitude},${loc.longitude},${loc.time}],`;
            });

            db.query("SELECT bd.*, pd.amt, pd.driver_amt, pd.tax_amt, pd.ride_commission, ud.push_token, pd.payment_type " +
                "FROM booking_detail AS bd " +
                "INNER JOIN payment_detail AS pd ON pd.payment_id = bd.payment_id " +
                "INNER JOIN user_detail AS ud ON ud.user_id = bd.user_id " +
                "WHERE bd.booking_id = ? AND bd.driver_id = ? AND bd.booking_status = ?", 
                [reqObj.booking_id, uObj.user_id, bs_start], (err, result) => {
                if (err) {
                    helper.ThrowHtmlError(err, res);
                    return;
                }
                ...
                const booking = result[0];
                const tollTax = parseFloat(reqObj.toll_tax) || 0;
                const finalAmount = parseFloat(booking.amt) + tollTax;

                const sqlUpdate = "UPDATE booking_detail AS bd " +
                    "INNER JOIN user_detail AS dd ON dd.user_id = bd.driver_id " +
                    "INNER JOIN user_detail AS ud ON ud.user_id = bd.user_id " +
                    "INNER JOIN payment_detail AS pd ON pd.payment_id = bd.payment_id " +
                    "SET bd.booking_status = ?, bd.toll_tax = ?, bd.drop_lat = ?, bd.drop_long = ?, bd.stop_time = NOW(), " +
                    "dd.status = 1, ud.status = 1, " +
                    "pd.amt = ?, pd.status = 1, pd.payment_date = NOW() " +
                    "WHERE bd.booking_id = ? AND bd.driver_id = ? AND bd.booking_status = ?";

                db.query(sqlUpdate, [
                    bs_complete,
                    tollTax,
                    reqObj.drop_latitude,
                    reqObj.drop_longitude,
                    finalAmount,
                    reqObj.booking_id,
                    uObj.user_id,
                    bs_start
                    ...
                    if (result.affectedRows > 0) {
                        bookingInformationDetail(reqObj.booking_id, '2', (status, details) => {
                            const payload = details[0];
                            const userSocket = controllerSocketList['us_' + payload.user_id];

                            if (userSocket && controllerIO.sockets.sockets.get(userSocket.socket_id)) {
                                controllerIO.sockets.sockets.get(userSocket.socket_id).emit("ride_stop", {
                                    status: "1",
                                    payload: {
                                        booking_id: parseInt(reqObj.booking_id),
                                        toll_tax: tollTax,
                                        amount: finalAmount,
                                        tax_amount: payload.tax_amt,
                            ...
                            oneSignalPushFire(1, [payload.push_token], nt_t_5_ride_complete, "Ride Complete", {
                                booking_id: reqObj.booking_id,
                                toll_tax: tollTax.toString(),
                                amount: finalAmount.toString(),
                                payment_type: payload.payment_type.toString(),
...
function driverNewRequestSend(bookingDetail, callback) {
    const latitude = parseFloat(bookingDetail.pickup_lat);
    const longitude = parseFloat(bookingDetail.pickup_long);

    helper.findNearByLocation(latitude, longitude, 20, async (minLat, maxLat, minLng, maxLng) => {
        const hasRequestedDrivers = bookingDetail.request_driver_id && bookingDetail.request_driver_id.trim() !== "";
        let additionalWhere = "";

        if (hasRequestedDrivers) {
            additionalWhere = `AND ud.user_id NOT IN (${bookingDetail.request_driver_id})`;
        }
        ...
        const sql = `
            ...
            INNER JOIN zone_document AS zd ON zd.zone_id = ud.zone_id 
                AND zd.service_id = ? 
                AND FIND_IN_SET(zd.service_id, ud.select_service_id) != 0 
            INNER JOIN price_detail AS pm ON pm.zone_id = zd.zone_id AND pm.price_id = ?
            INNER JOIN zone_wise_cars_service AS zwcs 
                ON ud.car_id = zwcs.user_car_id AND zwcs.zone_doc_id = zd.zone_doc_id 
            WHERE ud.user_type = 2 AND ud.status >= 1 AND ud.is_request_send = 0 
                AND ud.is_online = 1 
                AND zwcs.expiry_date >= ? 
                AND zwcs.status = 1 AND zwcs.service_provide = 1 
                AND (ud.lati BETWEEN ? AND ?) 
                AND (ud.longi BETWEEN ? AND ?) 
                ${additionalWhere}
                AND ud.user_id NOT IN (
                    SELECT driver_id FROM booking_detail 
        ...
        db.query(
            ...
            async (err, result) => {
                if (err) return helper.ThrowHtmlError(err);

                if (!result || result.length === 0) {
                    await markBookingNoDriver(bookingDetail);
                    return callback(2, "No driver found");
                }

                // احسب المسافة ورتب
                result.forEach(driver => {
                    driver.distance = helper.distance(latitude, longitude, driver.lati, driver.longi);
                });
                result.sort((a, b) => a.distance - b.distance);

                tryNextDriver(bookingDetail, result, 0, callback);
            }
        );
    });
}

// دالة المحاولة التتابعية
function tryNextDriver(bookingDetail, driverList, index, callback) {
    if (index >= driverList.length) {
        return markBookingNoDriver(bookingDetail, callback);
    }

    const driver = driverList[index];
    const socketInfo = controllerSocketList['us_' + driver.user_id];
    const socket = socketInfo ? controllerIO.sockets.sockets.get(socketInfo.socket_id) : null;

    if (socket) {
        driverSendRequestFire(bookingDetail, driver, true);
        ...
        socket.emit("new_ride_request", response);

        // ننتظر لمدة محددة قبل تجربة التالي
        setTimeout(() => {
            // تحقق إذا تم قبول الطلب أم لا
            db.query("SELECT booking_status FROM booking_detail WHERE booking_id = ?", [bookingDetail.booking_id], (err, res) => {
                if (err || res.length === 0) {
                    return callback(2, "Booking not found");
                }

                const status = res[0].booking_status;
                if (status > bs_pending)
                {
                    return callback(1, bookingDetail); // تم قبول الطلب
                } else {
                    tryNextDriver(bookingDetail, driverList, index + 1, callback); // جرّب التالي
                }
            });
        }, 12000); // 12 ثانية انتظار (يمكنك تغييره)
    } else {

        helper.insertBookingEventLog(
    bookingDetail.booking_id,
    "SOCKET_EMIT_FAIL",
    `السائق ${driver.user_id} غير متصل بالسوكيت`,
    {
    ...
    driver.user_id
        ...
        // السائق غير متصل فعليًا بالسوكيت    // تابع للسائق التالي
        tryNextDriver(bookingDetail, driverList, index + 1, callback);
       // 🟠 تسجيل فشل السوكيت
      //  helper.insertBookingLog(bookingDetail.booking_id, 'SOCKET_FAILED', driver.user_id, null, 'Socket not connected for driver', driver);
...
function driverSendRequestFire(bookingDetail, driverDetail, isSendNotification) {
 ...
 helper.insertBookingEventLog(
    bookingDetail.booking_id,
    "REQUEST_SENT",
    `تم إرسال الطلب إلى السائق ${driverDetail.user_id}`,
    {
        request_token: requestToken,
        driver_id: driverDetail.user_id,
        booking_id: bookingDetail.booking_id,
        push_token: driverDetail.push_token
    },
    bookingDetail.user_id,
...
function driverSendRequestTimeOver(bookingId, driverId, requestToken, time) {
    ...
    var oneTimeCron = setTimeout(() => {
       ...
       // helper.insertBookingLog(bookingId, 'DRIVER_TIMEOUT', driverId, null, 'Driver did not respond within timeout', {});
    helper.insertBookingEventLog(
    bookingId,
    "DRIVER_TIMEOUT",
    `السائق ${driverId} لم يرد خلال المهلة المحددة (${time} ثانية)`,
    {
        driver_id: driverId,
    ...
    driverId
        ...
        db.query("UPDATE `user_detail` SET `is_request_send` = ? WHERE `user_id` = ? ", ['0', driverId], (err, result) => {
            ...
            if (result.affectedRows > 0) {
                 ...
                 bookingId,
                    "REQUEST_RETRY",
                     `محاولة إعادة إرسال الطلب إلى سائق جديد بعد انتهاء مهلة السائق ${driverId}`,
                      {
                ...
                bookingInformation(booking_id, user_type, (status, result) => {
                    if (status == 1) {
                        ...
                        helper.timeDuration(rideCancelTime, helper.serverMySqlDate(result[0][checkTime]), (totalMin, durationString) => {
                             // ✅ إعادة is_request_send للسائق إذا ألغى الراكب الرحلة
                           if (user_type == ut_user && result[0].driver_id) {
                             db.query("UPDATE user_detail SET is_request_send = '0' WHERE user_id = ?", [result[0].driver_id], (err2) => {
                           if (err2) {
                              helper.Dlog("Failed to reset is_request_send for driver: " + err2.message);
                            } else {
                                helper.Dlog("Reset is_request_send = 0 for driver_id: " + result[0].driver_id);
                            }
                            });
                            }
                            if (booking_status >= bs_go_user && booking_status <= bs_wait_user) {
...
// تعيين الحالة إلى لا يوجد سائق متاح

async function markBookingNoDriver(bookingDetail, callback = () => {}) {
    // 1. تسجيل الحدث في السجل
    helper.insertBookingEventLog(
        bookingDetail.booking_id,
        "NO_DRIVER_AVAILABLE",
        `لم يتم العثور على أي سائق متاح`,
        {
            booking_id: bookingDetail.booking_id,
            tried_drivers: bookingDetail.request_driver_id || null,
            pickup_location: bookingDetail.pickup_address,
    ...
    // 2. تحديث حالة الحجز
    db.query(
        "UPDATE booking_detail SET booking_status = ?, stop_time = NOW() WHERE booking_id = ?",
        [bs_no_driver, bookingDetail.booking_id],
        async (err) => {
            if (err) {
                console.error("❌ خطأ في تحديث حالة الحجز:", err);
                return callback(2, "خطأ في تحديث الحجز");
            }

            try {
                // 3. جلب جميع السائقين المؤهلين مع عدد الطلبات المعلقة
                db.query(
                    `SELECT 
                        ud.user_id,
                        ud.name,
                        ...
                        (SELECT COUNT(*) FROM booking_detail 
                         WHERE booking_status = ${bs_no_driver} 
                         AND pickup_date >= NOW() - INTERVAL 5 MINUTE) AS pending_requests
                     FROM user_detail ud
                     LEFT JOIN driver_push_log dpl ON ud.user_id = dpl.driver_id
                     WHERE ud.user_type = 2`,
                    async (err, drivers) => {
                        if (err) {
                            console.error("❌ خطأ في جلب السائقين:", err);
                            return callback(2, "خطأ في جلب البيانات");
                        }

                        console.log("🔍 عدد السائقين في النظام:", drivers.length);

                        const now = new Date();
                        const FIVE_MINUTES = 5 * 60 * 1000;

                        const eligibleDrivers = drivers.filter(driver => {
                            const lastPush = driver.last_push_time ? new Date(driver.last_push_time) : null;

                            if (driver.status !== 1 || driver.is_block !== 0 || !driver.push_token || driver.push_token === '') {
                                return false;
                            }

                            if (driver.is_online === 1 && driver.is_request_send === 0) {
                                return true;
                            }

                            if (driver.is_online === 0 && driver.is_request_send === 0 && (!lastPush || (now - lastPush) > FIVE_MINUTES)) {
                                return true;
                            }
                        ...
                        console.log("🚗 السائقون المرشحون للإشعار:", eligibleDrivers.map(d => ({
                            id: d.user_id,
                            name: d.name,
                            online: d.is_online,
                            available: d.is_request_send === 0,
                            last_push: d.last_push_time
                        })));

                        if (eligibleDrivers.length === 0) {
                            const allRejected = drivers.every(driver => {
                                const lastPush = driver.last_push_time ? new Date(driver.last_push_time) : null;
                                ...
                                return (
                                    ...
                                    driver.push_token === '' ||
                                    (
                                        (driver.is_online === 0 || driver.is_request_send !== 0) &&
                                        lastPush && (now - lastPush) <= FIVE_MINUTES
                                    )
                                );
                            });

                            if (allRejected) {
                                console.log("⚠️ لا يوجد سائقون متاحون (كلهم لا يلبون الشروط)");
                                return callback(2, "لا يوجد سائقون متاحون");
                            }

                            console.log("⏳ لا يوجد مرشحين حاليًا لكن بعض السائقين قد يكونوا مؤهلين لاحقًا");
                            return;
                        }

                        // إذا وصلنا هنا، هناك سائقون مؤهلون ونرسل الإشعار
                        const tokens = eligibleDrivers.map(d => d.push_token).filter(Boolean);
                        const pendingCount = drivers[0]?.pending_requests || 1;

                        try {
                            const notificationResult = await oneSignalPushFire(
                                2,
                                tokens,
                                `🚖 ${pendingCount} طلب بانتظارك`,
                                "يوجد طلبات جديدة في منطقتك، يرجى فتح التطبيق",
                                {
                                    type: "driver_alert",
                                    booking_id: bookingDetail.booking_id,
                            ...
                            const updatePromises = eligibleDrivers.map(driver => {
                                return new Promise((resolve) => {
                                    db.query(
                                        `INSERT INTO driver_push_log (driver_id, last_push_time) 
                                         VALUES (?, NOW()) 
                                         ON DUPLICATE KEY UPDATE last_push_time = NOW()`,
                                        [driver.user_id],
                                        (err) => {
                                            if (err) console.error("⚠️ خطأ في تحديث سجل السائق:", driver.user_id, err);
                                            resolve();
                                        }
                                    );
                                });
                            ...
                            console.log(`✅ تم إرسال الإشعارات بنجاح إلى ${eligibleDrivers.length} سائق`);
                            console.log("📊 نتيجة الإشعار:", notificationResult);
                            callback(1, "تم إرسال الإشعارات بنجاح");
                        } catch (error) {
                            console.error("❌ فشل إرسال الإشعارات:", {
                                error: error.message,
                                stack: error.stack
                            });
                            callback(2, "فشل إرسال الإشعارات");
                        }
                    }
                );
            } catch (error) {
...
async function oneSignalPushFire(userType, tokenArray, title, message, messageData = {}) {
    const ONESIGNAL_APP_ID = "d8808a92-c4df-4c8b-a405-ae5822a0c946";
    const ONESIGNAL_API_KEY = "os_v2_app_3caivewe35gixjafvzmcfigjizmh3dmfkwqumw5axrwscm4kqxye4h47ir6k4k34dwvko3kpimv2jle7aaddql7wmoafnerlg7sjrxa";

    // 1. تنظيف ومعالجة التوكنات
    ...
    if (filteredTokens.length === 0) {
        console.error("⚠️ لا توجد توكنات صالحة للإرسال");
        return { success: false, message: "No valid push tokens" };
    }

    // 2. تحضير بيانات الإشعار
    const payload = {
        app_id: ONESIGNAL_APP_ID,
        include_player_ids: filteredTokens,
    ...
    try {
        ...
        return {
            ...
            data: result,
            meta: {
                tokens_sent: filteredTokens.length,
                timestamp: new Date().toISOString()
            }
        };
        ...
        console.error("💥 فشل إرسال الإشعار:", {
            ...
            payload: {
                ...payload,
                include_player_ids: ['تم حذف التوكنات لأسباب أمنية']
            },
            timestamp: new Date().toISOString()
        });
        ...
        return {
            success: false,
            error: error.message,
            retryable: !error.message.includes('AbortError')
        };
    }
}
// أضف هذا الاختبار في الكود
async function testOneSignal() {
    try {
        const response = await fetch('https://onesignal.com', { method: 'HEAD' });
        console.log("✅ اتصال ناجح بـ OneSignal");
        return response.ok;
    } catch (error) {
        console.error("❌ فشل الاتصال بـ OneSignal:", error.message);
        return false;
    }
}

// function oneSignalPushFire(userType, tokenArray, title, message, messageData = {}) {
//     const ONESIGNAL_APP_ID = "d8808a92-c4df-4c8b-a405-ae5822a0c946";
//     const ONESIGNAL_API_KEY = "os_v2_app_3caivewe35gixjafvzmcfigjizmh3dmfkwqumw5axrwscm4kqxye4h47ir6k4k34dwvko3kpimv2jle7aaddql7wmoafnerlg7sjrxa";

//     const filteredTokens = Array.isArray(tokenArray)
//         ? tokenArray.filter(t => typeof t === 'string' && t.trim() !== '')
//         : [];
...
//     console.log("🔔 Filtered Push Tokens:", filteredTokens);
//     console.log("📦 OneSignal Payload Data:", safeMessageData);
...
//     axios.post("https://onesignal.com/api/v1/notifications", postData, {
//         headers: {
//             "Content-Type": "application/json",
//             "Authorization": `Basic ${ONESIGNAL_API_KEY}`
//         }
...
//         insertBookingEventLog(
//             safeMessageData.booking_id || "0",
//             "PUSH_SENT",
//             `Push sent successfully: ${title} - ${message}`,
//             { tokenArray: filteredTokens, title, message, messageData: safeMessageData },
//             userType == 2 ? safeMessageData.user_id || null : null,
//             userType == 1 ? safeMessageData.driver_id || null : null
//         );
//     })
//     .catch(err => {
