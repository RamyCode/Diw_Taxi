// File: taxi_driver_node-main/controllers/cars_controller.js
     ...
     app.post('/api/car_list', (req, res) => {
        checkAccessToken(req.headers, res, (uObj) => {
            ...
            db.query(
                ...
                '`uc`.`status`, `sd`.`service_name`, `sd`.`service_id`, `ud`.`select_service_id`, IFNULL(`zwcs`.`status`, 0) AS `service_status`, ' +
                ...
                'INNER JOIN `car_model` AS `cm` ON  `cs`.`model_id` = `cm`.`model_id` ' + // <-- تم التعديل هنا
                ...
                'GROUP BY `uc`.`user_car_id`, `cs`.`series_name`, `cm`.`model_name`, `cb`.`brand_name`, `uc`.`car_number`, `uc`.`car_image`, `uc`.`status`, `sd`.`service_name`, `sd`.`service_id`, `ud`.`select_service_id`, `zwcs`.`status`, `ud`.`car_id` ' +
