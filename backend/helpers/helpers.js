// File: taxi_driver_node-main/helpers/helpers.js
const db = require('./db_helpers');
...
module.exports = {
  ...
  insertBookingEventLog: insertBookingEventLog,
...
function insertBookingEventLog(booking_id, event_type, message, data_json = {}, user_id = null, driver_id = null) {
    const logObj = {
        ...
        created_at:serverYYYYMMDDHHmmss()
    };

    db.query("INSERT INTO booking_event_log SET ?", logObj, (err, result) => {
        if (err) {
            console.error("❌ فشل تسجيل booking_event_log:", err);
        } else {
            console.log("📝 سجلت booking_event_log:", event_type, "لـ booking_id:", booking_id);
        }
    });
}
